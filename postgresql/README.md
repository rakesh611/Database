...
PostgreSQL is a MVCC relational/object-relational database. This chapter set treats it as an operating system + database service rather than only as a SQL product.
...
|---|---|
| Database | PostgreSQL |
| Current baseline | 18.6 |
| Default port | 5432 |
| Service | `postgresql.service` |
| CLI | `psql` |
| Data directory | `/var/lib/pgsql/18/data` |
| Configuration | `postgresql.conf / pg_hba.conf` |
| Replication | Physical streaming replication; logical replication |
| HA | Streaming replication plus an external failover/orchestration layer |
| Backup | pg_dump/pg_restore for logical backups; pg_basebackup for physical base backups; WAL for PITR |
...
Use PostgreSQL when its consistency model, ecosystem, operational model and application compatibility fit the workload. Evaluate workload requirements rather than choosing a database from popularity alone.
